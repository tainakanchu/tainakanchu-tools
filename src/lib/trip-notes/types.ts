...
export interface Place {
  ...
  localName?: string
  /**
   * ラテン文字表記。外部の検索サイトに渡すためのもので、画面表示用ではない。
   *
   * ■ localName と何が違うのか
   *   localName は「その土地の人が読む文字」で、見せる相手は現地の人間である
   *   (タイならタイ文字、ギリシャならギリシャ文字が入る)。
   *   latinName は「外部サービスが解決できる文字」で、渡す相手は機械である。
   *   目的が違うので、どちらか一方で兼ねることはできない。
   *   日本人の利用者は name に日本語しか書かないため、この欄が無いと
   *   Rome2Rio のような英語圏のサービスに「香港国際空港」がそのまま渡り、
   *   検索が成立しない(searchLinks.ts)。latinName さえあれば、
   *   日本語の地名しか持たない予約でも外部リンクが機能する。
   *
   * ■ なぜ enName ではないのか
   *   英語名とは限らないため。フランスの駅のように英語名を持たない場所でも
   *   ラテン文字の表記は必ずあり、外部サイトが解決できるのはそちらである。
   *   「英語に訳せ」と読める名前にすると、訳が無い場所で埋められなくなる。
   */
  latinName?: string
  address?: string
  lat?: number
...
export interface Booking {
  ...
  /**
   * オンラインチェックインが開く時刻。出発の何分前かを分で持つ(1440 = 24 時間前)。
   * 相対値で持つ理由は checkInClosesMinutesBefore と同じ。
   *
   * ■ なぜ「開く時刻」をわざわざ持つのか
   *   航空会社はオンラインチェックインを出発の 24 / 48 / 72 時間前に開放し、
   *   窓側・非常口前・前方といった良い座席は早い者勝ちで減っていく。
   *   開いた瞬間に席を取りたい人にとっては、この時刻こそが動くべき時刻になる。
   *   締切だけを持っていると「45 分前までに済ませればよい」としか言えず、
   *   良い席が残っているうちに動くという唯一の機会を、何も警告しないまま逃す。
   *
   * ■ 締切の 2 項目とは入れ物が同じでも扱いが違う
   *   出発からの相対分という持ち方は checkInClosesMinutesBefore と同じだが、
   *   こちらは「〜までにやる」ではなく「〜からできる」である。
   *   そのため時刻を過ぎたときは期限切れではなく「受付中」という状態に言い換え、
   *   残り時間の強調もしない(過ぎて困る類のものではない)。
   *   その言い換えと強調の判断の実体は milestones.ts にある。
   *
   * 妥当な範囲(整数・1 分以上・上限)の検証は storage.ts にあり、
   * 上限だけは締切より大きい(72 時間前の開放が実在するため。
   * storage.ts の MAX_CHECK_IN_OPENS_MINUTES_BEFORE を参照)。
   */
  onlineCheckInOpensMinutesBefore?: number
  /**
...
/**
 * 訪問先の国・地域の基本情報(プラグ形状・電圧・チップ・緊急通報番号)。
 *
 * ■ なぜ予約単位ではなく旅程単位なのか
 *   プラグ形状も緊急通報番号も、1 件の予約ではなく国に属する情報である。
 *   予約に持たせると同じ国に 3 泊すれば同じ「C タイプ・230V」を 3 回書くことになり、
 *   直すときも 3 箇所を直すことになる。しかも移動の予約は出発地と到着地で
 *   国が変わりうるので、どちらの国の値なのかが型の上で決まらない。
 *   旅程に 1 件ずつ持てば、どちらの問題も起きない。
 *
 * ■ なぜ TravelDoc に混ぜないのか
 *   TravelDoc は「出発前に済ませる手続き」で、状態(todo/applied/done)と期限を持ち、
 *   進捗タブの警告(docs.ts)の対象になる。国の基本情報は済ませるものではなく、
 *   期限も進捗も無い。同じ入れ物に入れると型の上では状態を持ててしまい、
 *   「未了のプラグ形状」という意味の無い警告を出せる形になる。
 *   出せる形にしておいて運用で出さないより、最初から別の型にする。
 *
 * ■ 国名(name)だけは人間が入れる
 *   予約の地名から訪問国を機械が推定することはしない。空港名から引ける場所もあれば、
 *   「知人宅」「Airbnb」のように何も引けない場所もあり、外したときは
 *   間違った国の緊急通報番号を見せることになる。行き先は利用者が確実に知っている
 *   のだから、そこは聞く。この割り切りは backfillPrompt.ts 側でも効いていて、
 *   AI には「国名は与えられたものを使う」前提で残りの欄だけを埋めさせる。
 *
 * ■ 緊急通報番号を警察と救急で分けて持つ理由
 *   国によって番号の分かれ方が違う。米国のように 911 で共通の国もあれば、
 *   イタリアのように警察(113)と救急(118)で別々の国もある。
 *   1 欄にまとめて「112 / 113」と書かせると、いちばん慌てているときに
 *   どちらが何なのかを読み解かせることになる。欄を分けておけば、
 *   画面のラベルのほうが答えを持っている。
 */
export interface CountryInfo {
  id: string
  /** 国・地域名。「マルタ」「台湾」など利用者の言葉でよい */
  name: string
  /** ラテン文字表記(検索やAI照合の補助) */
  latinName?: string
  /** プラグ形状 (例: 'A / C' や 'BF') */
...
export interface TripNotesState {
  ...
  /**
   * 訪問先の国・地域の基本情報(プラグ形状・緊急通報番号など)。
   *
   * placeAliases / travelDocs とまったく同じ理由で必須にせず任意にしている
   * (保存済みの localStorage にも発行済みの共有URLにもこのフィールドは無く、
   *  1 件も登録しない利用者の JSON や共有URLを `"countryInfos":[]` で
