 ...
 *   ここで付けるのは、利用者が予約に対して明示的に入力した時刻
 *   (オンラインチェックインの開始・搭乗手続きの締切・受託手荷物の締切)だけに限る。
 *   どれも「知らないと乗れない」「知らないうちに機会が終わる」種類の情報で、
 *   カレンダー側の既定通知では代替できない。
 *
...
/**
 * アラーム 1 つ。分数が入っていなければ何も作らない。
 * 締切(搭乗手続き・受託手荷物)にもオンラインチェックインの開始にも使う。
 * どちらも「出発の何分前か」で持っているので、TRIGGER の作り方は変わらない。
 *
 * 種別で絞らないのは、AI 取り込みが手段未定の移動を kind: 'other' に
 * 分類するため(itinerary.ts の isMoveBooking)。利用者がこの分数を
...
function bookingEvent(booking: Booking, dtstamp: string): Array<string> | null {
  ...
  /*
    オンラインチェックインの開始と締切のアラームは、時刻付きの予約にだけ付ける。
    終日の予約の DTSTART は現地 00:00 なので、そこから 60 分前を計算すると
    前日の 23:00 というもっともらしい嘘の時刻に鳴る。出発時刻そのものが
    分かっていない予約で、そこからの相対時刻だけ正しく置くことはできない
    (types.ts でこれらを相対値で持っているのと同じ話で、
    嘘の時刻を出すくらいなら出さないほうがよい)。

    並びは時系列。オンラインチェックインが開き、手荷物が締まり、
    最後に搭乗手続きが締まる順に読める。
  */
  if (!window.allDay) {
    lines.push(
      /*
        オンラインチェックインの開始は締切ではないが、この 3 つの中で
        カレンダーの通知がいちばん効くのはむしろこれである。
        この項目に価値があるのは「開いた瞬間に席を取る」という 1 点だけで、
        24 時間前という時刻を自力で思い出せる人はほとんどいない。
        通知はまさにその瞬間を拾うための道具なので、締切と同じ入れ物に載せる。

        ただし性質は締切と違う。鳴り損ねても便には乗れて、失うのは
        良い席の選択肢だけである。だから「鳴らないと詰む」種類の通知として
        重ねて出したり、強調したりはしない(milestones.ts の扱いと揃える)。
      */
      ...alarmLines(
