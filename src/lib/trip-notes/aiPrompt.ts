 ...
 *
 * 締切の 2 項目(搭乗手続き・受託手荷物)だけはその例外として
 * 「調べて埋めてよい」と書いてある。これは規則を緩めたのではなく、
 * 対象の性質が違うためで、例外だと分かる形(ルール 1 からの参照 + 独立した
 * ルール 8)で書いてある。理由もプロンプトの中に書いてあり、
...
1. **不明な項目は必ず null にしてください。推測で埋めないこと。**
   ...
   (唯一の例外は締切の 2 項目です。ルール 8 を参照してください)
